        ...
        function initializeTable() {
            variantsTable = new Tabulator("#variants-table", {
                ...
                paginationSizeSelector: [10, 25, 50, 100,
                    true
                ], // Cho phép chọn số lượng trên mỗi trang
                ...
                columns: [{
                    ...
                    // <<< CỘT THAO TÁC ĐÃ ĐƯỢC CẬP NHẬT
                    {
                        title: "Thao Tác",
                        hozAlign: "center",
                        width: 160, // <<< THAY ĐỔI 1: Tăng chiều rộng để chứa thêm nút
                        headerSort: false,
                        frozen: true, // <<< THAY ĐỔI 2: Cố định cột ở bên phải
                        formatter: (cell) =>
                            `<i class="fa-solid fa-edit action-icon icon-edit" title="Sửa Biến thể"></i>
                             ...
                             <i class="fa-solid fa-trash-can action-icon icon-delete" title="Xóa Biến thể"></i>`, // <<< THAY ĐỔI 3: Thêm icon xóa
                        cellClick: (e, cell) => {
                            e.stopPropagation();
                            const data = cell.getRow().getData();
                            // <<< THAY ĐỔI 4: Thêm logic xử lý nút xóa
                                ...
                                if (confirm(
                                    ...
                                    apiRequest('delete_multiple_variants', {
                                        ...
                                        .then(() => {
                                            ...
                                            loadAllVariants(); // Tải lại bảng sau khi xóa
                                        })
                                        .catch(error => {
                                            // Lỗi đã được hiển thị bởi hàm apiRequest
        ...
        async function openVariantForm(variantId = null) {
            ...
            const updateAttributeVisibility = () => {
                ...
                if (skuPrefix === 'CV' || skuPrefix === 'CT') {
                    ...
                    attributeFormGrid.querySelector('[data-attribute-name-group="Kích thước ren"]')
                        ?.classList.remove('hidden');
                        attributeFormGrid.querySelector('[data-attribute-name-group="Trọng lượng"]')
                        ?.classList.remove('hidden');
                }
            };
            ...
            form.addEventListener('click', async e => {
                if (e.target.classList.contains('add-option-btn')) {
                    ...
                    const callback = () => {
                        openVariantForm(
                            currentVariantId
                        ); // Gọi lại openVariantForm để khôi phục và cập nhật
